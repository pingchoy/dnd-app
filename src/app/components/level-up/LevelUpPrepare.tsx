...
export default function LevelUpPrepare({
  availableSpells,
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="font-crimson text-parchment/50 italic text-base mt-1">
          Choose which spells to prepare for the day. You can change your prepared spells at each level-up.
        </p>
        <div className="mt-2">
          <span
          ...
          >
            {selectedPreparedSpells.length} / {maxPreparedSpells} prepared
          </span>
        </div>
      </div>
      ...
      {availableSpells.map((spellsAtLevel, idx) => {
        ...
        return (
          <section key={spellLevel}>
            <h3 className="font-cinzel text-xs text-parchment/40 tracking-widest uppercase mb-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {spellsAtLevel.map((spell) => (
                <SpellCard
                  key={spell.slug}
                  spell={spell}
                  isSelected={selectedPreparedSpells.includes(spell.slug)}
                  disabled={selectedPreparedSpells.length >= maxPreparedSpells}
                  onToggle={() => onToggle(spell.slug)}
