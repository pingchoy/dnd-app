...
export function OrnateFrame({
  ...
  const flourishHover =
    isStatic || selected || !hasHover ? "" : "group-hover:text-gold/60";
  ...
  return (
    <div
      className={`relative p-[6px] ${openBottom ? "pb-0" : ""} ${className ?? "h-full"}`}
    >
      {/* Outer border */}
      <div
        className={`absolute inset-0 ${outerRadius} border ${openBottom ? "border-b-0" : ""} ${outerBorder} transition-colors`}
      />
      {/* Inner border */}
      <div
        className={`absolute inset-[3px] ${openBottom ? "bottom-0" : ""} ${innerRadius} border ${openBottom ? "border-b-0" : ""} ${innerBorder} transition-colors`}
      />

      {/* Corner flourishes */}
      <CornerFlourish
        className={`absolute top-0 left-0 ${flourishColor} ${flourishHover} transition-colors`}
      />
      <CornerFlourish
        className={`absolute top-0 right-0 -scale-x-100 ${flourishColor} ${flourishHover} transition-colors`}
      />
      {!openBottom && (
        <>
          <CornerFlourish
            className={`absolute bottom-0 left-0 -scale-y-100 ${flourishColor} ${flourishHover} transition-colors`}
          />
          <CornerFlourish
            className={`absolute bottom-0 right-0 -scale-x-100 -scale-y-100 ${flourishColor} ${flourishHover} transition-colors`}
          />
        </>
      )}
      ...
      {/* Card content */}
      <div
        className={`relative ${innerRadius} overflow-hidden h-full flex flex-col ${
