...
export async function POST(req: NextRequest) {
  try {
    ...
    const { characterId, singleTargetResult, npcResults } = body;
    ...
    if (singleTargetResult.noCheck) {
      playerTurnText = `PLAYER ACTION: ${singleTargetResult.checkType} — ${singleTargetResult.notes}`;
    } else if (singleTargetResult.impossible) {
      playerTurnText = `PLAYER ACTION FAILED: ${singleTargetResult.notes}`;
    } else {
      const hitMiss = singleTargetResult.success ? "HIT" : "MISS";
      playerTurnText = `PLAYER TURN: ${singleTargetResult.checkType}, rolled ${singleTargetResult.dieResult}${singleTargetResult.totalModifier}=${singleTargetResult.total} vs AC ${singleTargetResult.dcOrAc} → ${hitMiss}`;
      if (singleTargetResult.damage) {
        const dmgBreakdown = singleTargetResult.damage.breakdown
          ...
          .join("; ");
        playerTurnText += `. Damage: ${singleTargetResult.damage.totalDamage} (${dmgBreakdown})`;
        if (singleTargetResult.damage.isCrit)
          playerTurnText += " CRITICAL HIT!";
      }
    }
    ...
    const systemPrompt = `You are a D&D 5e Dungeon Master narrating combat. Write in second person — address the player as "you". Given the mechanical results below, write vivid 1-2 paragraph narration.
