...
export async function narratePlayerTurn(
  ...
  if (playerResult.noCheck) {
    prompt += `PLAYER ACTION: ${playerResult.checkType} — ${playerResult.notes}`;
  } else if (playerResult.impossible) {
    prompt += `PLAYER ACTION FAILED: ${playerResult.notes}`;
  } else {
    const hitMiss = playerResult.success ? "HIT" : "MISS";
    prompt += `PLAYER TURN: ${playerResult.checkType}, rolled ${playerResult.dieResult}${playerResult.totalModifier}=${playerResult.total} vs AC ${playerResult.dcOrAc} → ${hitMiss}`;
    if (targetNPC) {
    ...
    if (playerResult.damage) {
      const dmgBreakdown = playerResult.damage.breakdown
        ...
        .join("; ");
      prompt += `. Damage: ${playerResult.damage.totalDamage} (${dmgBreakdown})`;
      if (playerResult.damage.isCrit) prompt += " CRITICAL HIT!";

      // Check if NPC died
      if (targetNPC && playerResult.damage.totalDamage >= targetNPC.currentHp) {
